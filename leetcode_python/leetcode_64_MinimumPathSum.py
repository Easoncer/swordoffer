# encoding: utf-8

'''

@author: liangchi

@contact: bnu_llc@163.com

@software: pycharm

@file: leetcode_64_MinimumPathSum.py

@time: 2018/4/18 下午12:55

'''

res = float('Inf')

def dfstofindpath(grid, mark, x, y, sum):
    global res
    if x == len(grid)-1 and y == len(grid[0])-1:
        if sum<res:
            res = sum
        return
    if x < len(grid)-1 and mark[x+1][y] == 0:
        if sum+grid[x+1][y] > res:
            print 'usefor'
            return
        mark[x+1][y] = 1
        dfstofindpath(grid, mark, x+1, y, sum+grid[x+1][y])
        mark[x+1][y] = 0

    if y < len(grid[0])-1 and mark[x][y+1] == 0:
        if sum+grid[x][y+1] > res:
            return
        mark[x][y+1] = 1
        dfstofindpath(grid, mark, x, y+1, sum+grid[x][y+1])
        mark[x][y+1] = 0

def minPathSum(grid):
    """
    :type grid: List[List[int]]
    :rtype: int
    """
    #mark = [[0]*len(grid[0])]*len(grid)


    ######################
    #####  backDFS   #####
    ###   time limit   ###
    ######################

    x = 0
    y= 0
    sum = grid[0][0]
    global res
    mark = [[0 for i in range(len(grid[0]))] for j in range(len(grid))]
    mark[0][0] = 1

    dfstofindpath(grid, mark, x, y, sum)
    return res



if __name__ == '__main__':
      print minPathSum([[5,0,1,1,2,1,0,1,3,6,3,0,7,3,3,3,1],[1,4,1,8,5,5,5,6,8,7,0,4,3,9,9,6,0],[2,8,3,3,1,6,1,4,9,0,9,2,3,3,3,8,4],[3,5,1,9,3,0,8,3,4,3,4,6,9,6,8,9,9],[3,0,7,4,6,6,4,6,8,8,9,3,8,3,9,3,4],[8,8,6,8,3,3,1,7,9,3,3,9,2,4,3,5,1],[7,1,0,4,7,8,4,6,4,2,1,3,7,8,3,5,4],[3,0,9,6,7,8,9,2,0,4,6,3,9,7,2,0,7],[8,0,8,2,6,4,4,0,9,3,8,4,0,4,7,0,4],[3,7,4,5,9,4,9,7,9,8,7,4,0,4,2,0,4],[5,9,0,1,9,1,5,9,5,5,3,4,6,9,8,5,6],[5,7,2,4,4,4,2,1,8,4,8,0,5,4,7,4,7],[9,5,8,6,4,4,3,9,8,1,1,8,7,7,3,6,9],[7,2,3,1,6,3,6,6,6,3,2,3,9,9,4,4,8]])


